...
def get_unary_qe_tb_energy():
    ...
    return loadjson(os.path.join(os.path.dirname(__file__), "unary_qe_tb.json"))
...
def get_twod_defect_energy(vrun="", jid="", atom=""):
    ...
    def get_enp_jid(jid=""):
        for i in dft2d:
            if i["jid"] == jid:
                return (
                    i["optb88vdw_total_energy"] / Atoms.from_dict(i["atoms"]).num_atoms
                )
...
class PhaseDiagram:
    ...
    def __init__(
        self, entries, verbose=False, only_plot_stable=False, only_label_stable=False,
    ):
        ...
        if verbose:
            ...
            for i, (count, energy, name, natoms) in enumerate(self.entries_dict):
    ...
    def get_ehull_all(self):
        ...
        for i in self.entries:
            # print('ent',i)
            ehull, energy, indices, coefs = self.energy_above_hull(entry=[i[0], i[1]])
            info.append([i, ehull])
        return info
    ...
    def plot2d2(self, ax=None):
        """Get 2D plot."""
        x, e = self.points[:, 1:].T
        names = [re.sub(r"(\d+)", r"$_{\1}$", ref[2]) for ref in self.entries_dict]
        hull = self.hull
